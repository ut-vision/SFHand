...
def get_video_reader(videoname, num_threads, fast_rrc, rrc_params, fast_rcc, rcc_params):
    if '/mnt/petrelfs' in videoname or client is None:
        if fast_rrc:
...
def video_loader(root, vid, ext, second, end_second,
    ...
    if chunk_len == -1:
        ...
        assert res.shape[0] == clip_length, "{}, {}, {}, {}, {}, {}, {}".format(root, vid, second, end_second,
                                                                                res.shape[0], rel_frame_ids, frame_ids)
        return res
...
def video_loader_by_timestamp(root, vid, start_timestamp=0, end_timestamp=0,
    ...
    try:
        ...
        print(error)
        print("Erroneous video: ", vid, start_timestamp, end_timestamp, start_frame, end_frame, fps, frame_ids,
              len(vr) - 1)
        frames = [torch.zeros((240, 320, 3)) for _ in range(len(frame_ids))]
