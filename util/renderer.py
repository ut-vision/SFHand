...
def render_one_hand(renderer, img, img_hw, verts, camt, is_right, focal, box=None, draw_box=True):
    h, w = img_hw
    verts[:, 0] = (2 * is_right - 1) * verts[:, 0] # Flip left hand
    ...
    misc_args = dict(
        ...
        focal_length=focal / 540. * min(h, w),
    )
    cam_view = renderer.render_rgba_multiple([verts], cam_t=[camt], render_res=(w, h),
                                                    is_right=[is_right], **misc_args)
    # Overlay image
    input_img = img.astype(np.float32) / 255.0
    input_img = np.concatenate([input_img, np.ones_like(input_img[:, :, :1])], axis=2)  # Add alpha channel
    input_img_overlay = input_img[:, :, :3] * (1 - cam_view[:, :, 3:]) + cam_view[:, :, :3] * cam_view[:, :, 3:]
    return input_img_overlay * 255.0
...
class Renderer:
    ...
    def __call__(self,
        ...
        if full_frame:
            ...
            image = image.clone() * torch.tensor(IMAGE_STD, device=image.device).reshape(3, 1, 1)
            image = image + torch.tensor(IMAGE_MEAN, device=image.device).reshape(3, 1, 1)
